...
from knowledge_base import ingest_text_to_knowledge_base
...
def process_to_knowledge_base(content: str, filename: str) -> dict:
    try:
        return ingest_text_to_knowledge_base(
            content,
            filename=filename,
...
def show_loading(filename: str, index: int, total: int) -> str:
    ...
    <div class="loading-box">
        ...
        <span class="loading-text">正在上传：{safe_name}（{index}/{total}）</span>
    </div>
    """
...
def render_upload_page() -> None:
    ...
    st.markdown(
        ...
        <div class="hero">
            ...
            <p class="hero-sub">选择多个 TXT → 点击上传 → 自动去重、分段并写入向量库</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
    ...
    with left_col:
        ...
        st.markdown(
            '<p class="muted">先把文件加入待上传列表，再统一串行写入知识库。</p>',
            unsafe_allow_html=True,
        )

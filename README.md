...
1. 支持常见格式：`txt / md / csv / json / html / pdf / docx / xlsx / pptx` 等
2. 可一次多选，或分多次添加（先临时记录）
3. 点击「开始整理并入库」后串行处理：
   - 解析文件文本
   - 调用大模型总结整理
   - 写入 `store/` 与 `vector_store/`
4. 相同原文（按 SHA256）不会重复整理入库
5. 老版 `.doc / .xls / .ppt` 请先另存为 `.docx / .xlsx / .pptx`
...
3. 当前支持多种常见文档格式；旧版 `.doc/.xls/.ppt` 需先转换为新格式。

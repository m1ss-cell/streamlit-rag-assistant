...
def build_store_chunk(
    ...
    if raw_sha256:
        meta_lines.append(f"[raw_sha256: {raw_sha256}]")
    chunk = "\n".join(meta_lines) + "\n\n" + normalized
...
def is_raw_content_uploaded(raw_content: str) -> bool:
    """根据原始提取文本的 SHA256 判断是否已处理过该文件内容。"""
    raw_digest = content_hash(raw_content)
    if not raw_digest:
        return False
    return f"[raw_sha256: {raw_digest}]" in read_store()
...
def save_text_to_store(
    ...
    """
    将文本内容保存到本地 store 文件中。

    额外记录：上传时间、上传者、SHA256、可选 raw_sha256。
    - 若内容已存在：跳过写入
...
def ingest_uploaded_document(
    ...
    *,
    ...
    3. 整理后文本写入 store + 向量库
    """
    from document_refine import refine_document_text

    raw_normalized = normalize_content(raw_text)
    raw_hash = content_hash(raw_normalized)
    ...
    if is_raw_content_uploaded(raw_normalized) or is_content_uploaded(raw_normalized):
        return {
            "success": True,
            "skipped": True,
            "message": "该文件内容已处理过，跳过整理与入库",
            "content_hash": raw_hash,
            "raw_sha256": raw_hash,
            "chunk_count": 0,
            "refined": False,
    ...
    if refine_with_llm:
        refined_text = refine_document_text(raw_normalized, filename=filename)
        refined = True
        message_prefix = "已整理并"
    else:
        refined_text = raw_normalized
        refined = False
        message_prefix = "已"
    ...
    if result.get("success") and not result.get("skipped"):
        result["message"] = (
            f"{message_prefix}写入知识库（{result.get('chunk_count', 0)} 段）"
        )
    result["refined_preview"] = refined_text[:500]
    return result
...
if __name__ == "__main__":
    ...
    print("第一次入库:", ingest_uploaded_document(demo, "demo.txt"))
    print("第二次入库:", ingest_uploaded_document(demo, "demo_again.txt"))
